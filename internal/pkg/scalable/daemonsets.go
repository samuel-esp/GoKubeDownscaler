...
func (d *daemonSet) ScaleUp() (ScalingSummary, error) {
	...
	fromReplicas := values.StringReplicas("not present")
	if _, exists := d.Spec.Template.Spec.NodeSelector[labelMatchNone]; exists {
...
func (d *daemonSet) ScaleDown(_ values.Replicas) (ScalingSummary, error) {
